...
export default async function staffMembers(container) {
    ...
    // Local cache of members with account info (for caretaker)
    let membersWithAccounts = [];

    async function loadMembersWithAccounts() {
        if (role !== 'caretaker' || !defaultAptId) return;
        try {
            const res = await apiService.get(`/staff/members/apartment/${defaultAptId}/accounts`);
    ...
    // If coming from Staff Roles page, pre‑select that role
    ...
    async function loadMembers() {
        ...
        try {
            if (role === 'caretaker') {
                ...
                else throw new Error(res.message || 'Failed to fetch staff');
            }
        } catch (e) {
    ...
    // ----- MODALS (same as before, but no `require` calls) -----
